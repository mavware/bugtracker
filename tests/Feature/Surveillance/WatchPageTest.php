...
// capture.js makes [data-app-nav] inert while recording; the watch layout has one
// such region, its header.
test('the watch chrome carries the hook capture.js locks while recording', function () {
    $page = $this->get(route('watch.capture'));

    preg_match_all('/<[a-z-]+[^>]*\sdata-app-nav[=\s>]/i', $page->getContent(), $marked);

    expect($marked[0])->toHaveCount(1);
});
