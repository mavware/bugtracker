...
{{--        </div>--}}
        ...
        {{-- Nights this browser recorded before the user had an account, one row
             each so every night can be filed under its own room and customer as
             it is saved. Saving removes the local copy, so a row that reads "saved"
             is one claimed from its own report page, which keeps the copy it is
             showing. claim.js fills the list from the night store and hides the
             whole panel while the store holds nothing. The customers go in the
             config so the script can offer them per row. --}}
        <div
            id="claim-nights"
            class="hidden"
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <flux:heading size="lg" class="flex items-center gap-2">
                            ...
                            {{ __('Nights on this device') }}
                        </flux:heading>
                        <flux:text class="mt-1 text-sm">
                            <span data-claim="pending">
                                {{ __('This browser holds') }} <span data-claim="count">0</span> {{ __('nights recorded without an account. Save each one here, under its room and customer, so it counts towards trends and entry points.') }}
                            </span>
